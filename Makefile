...
bundle-tocify:
	# Build tocify bundle
	mkdir -p build
	NODE_PATH=$(NODE_PATH) $(GRUNT) bundle-tocify $(DEBUG) $(VERBOSE) --gruntfile=collective/tocify/js/Gruntfile.js
...
ifeq ($(NODE_VERSION_LT_011),true)
	...
	NODE_PATH=$(NODE_PATH) $(GRUNT) sed:bootstrap $(DEBUG) $(VERBOSE) --gruntfile=collective/tocify/js/Gruntfile.js

jshint:
	# Codecheck
	NODE_PATH=$(NODE_PATH) $(GRUNT) jshint $(DEBUG) $(VERBOSE) --gruntfile=collective/tocify/js/Gruntfile.js

watch:
	# Run grunt and watch for changes.
	NODE_PATH=$(NODE_PATH) $(GRUNT) watch $(DEBUG) $(VERBOSE) --gruntfile=collective/tocify/js/Gruntfile.js

test:
	# Run tests headless via PhantomJS and watch for changes.
	NODE_PATH=$(NODE_PATH) $(GRUNT) test $(DEBUG) $(VERBOSE) --pattern=$(pattern) --gruntfile=collective/tocify/js/Gruntfile.js

test-once:
	# Run the tests headless with PhantomJS only once and exit.
	NODE_PATH=$(NODE_PATH) $(GRUNT) test_once $(DEBUG) $(VERBOSE) --pattern=$(pattern) --gruntfile=collective/tocify/js/Gruntfile.js

test-dev:
	# Run the tests for Chromium and watch for changes.
	NODE_PATH=$(NODE_PATH) $(GRUNT) test_dev $(DEBUG) $(VERBOSE) --pattern=$(pattern) --gruntfile=collective/tocify/js/Gruntfile.js
